...
final class Writer
{
	...
	/**
	 * Appends the encoded ISDOC document and the remaining supplements to the carrier PDF as an incremental update.
	 *
	 * Each embedded file is written as an /EmbeddedFile stream, then exposed through a /Filespec and an /EmbeddedFiles
	 * name tree linked from the document catalog, so that conforming readers (Acrobat, pdf.js, poppler, PdfPig/PDFBox,
	 * …) discover the attachments the spec-defined way — through /Root → /Names → /EmbeddedFiles — instead of only via
	 * a brute-force scan for /EmbeddedFile streams. The stream still carries /F as well so this library's own scanning
	 * reader keeps the filename on round-trip.
	 *
	...
	private function append(string $filename, ISDOC\Schema\Invoice $invoice): void
	{
		...
		if (!preg_match('/^(\d+)\s+(\d+)\s+R$/', $root, $rootRef)) {
		...
		// The whole carrier is needed to locate the catalog dictionary (and, when present, the names dictionary it
		// references) so the new name tree can be merged into it without dropping the carrier's existing entries.
		...
		// Object numbers for the appended objects start past the carrier's highest. Each appended object's byte offset
		// is recorded in $xref as [offset, generation] for the new cross-reference section.
		$num  = $size;
		$xref = [];
		...
		// One /Filespec per embedded file. The name belongs here (not on the stream), and /EF points at the stream;
		// /AFRelationship associates it for PDF/A-3 / Factur-X-aware tooling. $names maps the name-tree key to the
		// filespec object number; keys must be unique, so sanitized-name collisions get a numeric suffix.
		$names = [];
		...
		// Link the tree into the catalog through /Names → /EmbeddedFiles, re-emitting whichever object actually owns
		// the names dictionary so the carrier's existing /Names entries (e.g. /Dests) are preserved.
		...
		if (preg_match('~/Names\s+(\d+)\s+(\d+)\s+R~', $catalog, $namesRef)) {

			// The catalog references a names dictionary indirectly; extend that object and leave the catalog as is.
		...
		} elseif (($namesPos = strpos($catalog, '/Names')) !== false) {
		...
		// Cross-reference section: contiguous object numbers grouped into subsections (the re-emitted carrier object
		// has a lower number than the appended ones, so it forms its own subsection).
